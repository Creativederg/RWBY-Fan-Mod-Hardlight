package dragonofcreativity.rwbyfm.objects.blocks.containers;
...
public class Hardlight_Forge_Container extends Container
{
    private final TileEntityHardlightForge tileentity;
    private int cookTime, totalCookTime, burnTime, currentBurnTime;

    public Hardlight_Forge_Container(InventoryPlayer player, TileEntityHardlightForge tileentity)
    {
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if(slot != null && slot.getHasStack())
        {
            ...
            else if(index != 2 && index != 1 && index != 0)
            {
                ...
                if(!HardlightForgeRecipe.getInstance().getHardlightForgeResult(stack1, slot1.getStack()).isEmpty())
                {
                    if(!this.mergeItemStack(stack1, 0, 2, false))
                    {
                        return ItemStack.EMPTY;
                    }
                    else if(TileEntityHardlightForge.isItemFuel(stack1))
                    {
