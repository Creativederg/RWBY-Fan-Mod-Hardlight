package dragonofcreativity.rwbyfm.objects.blocks.tileentities;

import javax.annotation.Nonnull;

import dragonofcreativity.rwbyfm.init.HardlightForgeRecipe;
...
public class TileEntityHardlightForge extends TileEntity implements ITickable
{
	private ItemStackHandler input = new HardlightForgeItemStackHandler(2) {
    	@Override
    	public boolean isItemValid(int slot, ItemStack stack) {
            if (slot == 0) {
                return true;
            } else if (stack.getItem() == RWBYItems.dustcrystalhardlight) {
                return stack.getItem() == RWBYItems.dustcrystalhardlight;
            } else return stack.getItem() == RWBYItems.dustrockhardlight;
        }
    };
    private ItemStackHandler fuel = new HardlightForgeItemStackHandler(1) {
    	@Override
    	public boolean isItemValid(int slot, ItemStack stack) {
    		return TileEntityHardlightForge.isItemFuel(stack);
    	}
    };
    private HardlightForgeItemStackHandler output = new HardlightForgeItemStackHandler(1) {
    	@Override
    	public boolean isItemValid(int slot, ItemStack stack) {
    		return false;
        }
    ...
    {
        return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.hardlight_forge");
    }
    ...
    public static boolean isBurning(TileEntityHardlightForge te)
    {
    ...
    public void update()
    {
        ...
        {
            --this.burnTime;
            RWBYFMHardlight_forge.setState(true, world, pos);
        }
        ...
        else
        {
            ...
            {
                ItemStack output = HardlightForgeRecipe.getInstance().getHardlightForgeResult(inputs[0], inputs[1]);
                if(!output.isEmpty())
                {
    ...
    private boolean canSmelt()
    {
        ...
        {
            ItemStack result = HardlightForgeRecipe.getInstance().getHardlightForgeResult((ItemStack)this.input.getStackInSlot(0), (ItemStack)this.input.getStackInSlot(1));
    ...
    private class HardlightForgeItemStackHandler extends ItemStackHandler {
        public HardlightForgeItemStackHandler()
        {
            super();
        }

        public HardlightForgeItemStackHandler(int size)
        {
            super(size);
        }

        public HardlightForgeItemStackHandler(NonNullList<ItemStack> stacks)
        {
            super(stacks);
        }
